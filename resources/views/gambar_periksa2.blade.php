			...
			<div class="row">
				<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
					{!! Form::file('foto_estetika[]') !!}
						@if (isset($image))
							<p> <img src="{{ \Storage::disk('s3')->url('img/estetika/'.$image->nama) }}" alt="" class="img-rounded upload"> </p>
						@else
	...
	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
		<div class="form-group @if($errors->has('keterangan_gambar[]'))has-error @endif">
		  {!! Form::label('keterangan_gambar[]', 'Ketarangan Gambar', ['class' => 'control-label']) !!}
			@if (isset($image))
				{!! Form::text('keterangan_gambar[]' , $image->keterangan, ['class' => 'form-control required']) !!}
			@else
