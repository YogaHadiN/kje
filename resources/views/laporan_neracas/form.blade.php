    <table class="table">
      <tbody>
				 ...
				 @foreach($akunAktivaLancar as $ju) 
					 @if($ju->total != 0)
						<tr>
							<td></td>
							<td>{{ $ju->id }} - {{ $ju->coa }}</td>
							<td class="text-right">{{App\Models\Classes\Yoga::buatrp(  $ju->total  )}}</td>
						</tr>
					@endif
				 @endforeach 
                <tr>
                ...
                <tr>
				 ...
				 @foreach($akunHutang as $v) 
					<tr>
					  <td></td>
					  <td>{{ $v->id }} - {{ $v->coa }}</td>
					  <td class="text-right">{{ App\Models\Classes\Yoga::buatrp($v->total) }}</td>
					</tr>
				 @endforeach 
                <tr>
				 ...
				 @endforeach 
					<tr>
					  ...
					  <td class="text-right">{{ App\Models\Classes\Yoga::buatrp($laba_tahun_berjalan) }}</td>
					</tr>
              </tbody>
            </table>
          </td>
		...
		<tr>
			<td><h1 class="text-right">{{ App\Models\Classes\Yoga::buatrp($total_harta) }}</h1></td>
          <td><h1 class="text-right">{{ App\Models\Classes\Yoga::buatrp($total_harta) }}</h1></td>
		</tr>
      </tbody>
    </table>
