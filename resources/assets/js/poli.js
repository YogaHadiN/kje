...
function refresh(){
	...
	$.post(base + '/poli/ajax/ambil_gambar', parameter, function (data) {
		...
		for (var i = 0, len = data.length; i < len; i++) {
			...
			temp += '<img src="' + base_s3 + '/img/estetika/' + data[i].image + '" class="img-thumbnails" alt="">'
			temp += '</div>'
			if (isOdd(i)) {
				temp += '</div><br />';
			}
...
function selectChange(control) {
    ...
    if (
		...
		html += '<input type="text" placeholder="hanya angka" class="form-control" id="keteranganTindakan" onkeypress="return angka(event)">';
	} else {
        html = '<input type="text" class="form-control" id="keteranganTindakan">'
    }
