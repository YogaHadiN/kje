    ...
    $("#ddlNamaObat").change(function () {
        ...
        $.post(
            ...
            function (data) {
                ...
                if (data == "1") {
                    alert(
                        "Ibuprofen syrup / tablet tidak boleh digunakan untuk anak dibawah umur 1 tahun, gunakan ibuprofen suppositoria dalam kondisi darurat!! Optimalkan pemberian paracetamol 15 mg/kgbb tiap 4 jam, disertai dengan ibuprofen supp dalam keadaan darurat !!"
                    );
                    $("#ddlNamaObat").val("").selectpicker("refresh");
                }
            }
...
function customOption(dataMerek) {
    ...
    for (var i = 0; i < dataMerek.length; i++) {
        if (dataMerek[i].merek_id == id) {
            temp +=
                '<option data-custom-value=\'{ "formula_id" : "' +
                dataMerek[i].formula_id +
                '", "rak_id" : "' +
                dataMerek[i].rak_id +
                '", "merek_id" : "' +
                dataMerek[i].merek_id +
                '", "harga_beli" : "' +
                dataMerek[i].harga_beli +
                '" , "aturan_minum_id" : "' +
                dataMerek[i].aturan_minum_id +
                '" , "harga_jual" : "' +
                dataMerek[i].harga_jual +
                '" , "sediaan" : "' +
                dataMerek[i].sediaan +
                '" , "tidak_dipuyer" : "' +
                dataMerek[i].tidak_dipuyer +
                '" }\' selected="selected" data-subtext=\'' +
                dataMerek[i].komposisi +
                "' value='" +
                dataMerek[i].merek_id +
                "' data-peringatan='" +
                dataMerek[i].peringatan +
                "' data-fornas='" +
                dataMerek[i].fornas +
                "' data-alternatif='" +
                dataMerek[i].alternatif +
                "'>" +
                dataMerek[i].merek +
                "</option>";
        } else {
...
function generatePerscription() {
    ...
    if (typeof result_array[i_sop_terapi] !== "undefined") {
