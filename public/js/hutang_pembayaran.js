...
$("body").on("keyup", ".ajaxGetPembayaran", function () {
    var colspan = $("#table_pembayaran_asuransi")
        .find("thead tr")
...
function getPembayaran(key = 0) {
    var displayed_rows = $("#table_pembayaran_asuransi")
        .closest(".panel-body")
        .find(".displayed_rows")
        .val();
    var param = {
        "pembayaran_asuransi_id ": $("#table_pembayaran_asuransi")
            .find(".id")
            .first()
            .val(),
        tanggal: $("#table_pembayaran_asuransi")
            .find(".tanggal_dibayar")
            .first()
            .val(),
        displayed_rows: displayed_rows,
        pembayaran: $("#table_pembayaran_asuransi")
            .find(".pembayaran")
            .first()
            .val(),
        nilai: $("#table_pembayaran_asuransi").find(".nilai").first().val(),
        selisih: $("#table_pembayaran_asuransi").find(".selisih").first().val(),
        deskripsi: $("#table_pembayaran_asuransi")
            .find(".deskripsi")
            .first()
            .val(),
    ...
    $.get(
        base + "/asuransis/riwayat/pembayaran/" + $("#asuransi_id").val(),
        param,
        function (data) {
            var temp = "";
            ...
            for (let i = 0, len = data.length; i < len; i++) {
                var selisih =
                    parseInt(data[i].pembayaran) - parseInt(data[i].nilai);
                // var tanggal = data[i].tanggal_dibayar == null ? '-': Date.parse(data[i].tanggal).toString("yyyy-MM-dd");
                temp += "<tr>";
                temp +=
                    "<td nowrap>" + data[i].pembayaran_asuransi_id + "</td>";
                temp += "<td nowrap>" + data[i].tanggal_dibayar + "</td>";
                temp +=
                    '<td class="uang" nowrap>' + data[i].pembayaran + "</td>";
            ...
            $("#container_pembayaran_asuransi")
                .find(".uang")
                .each(function () {
                    if ($(this).html() == "null" || $(this).html() == "NaN") {
                        $(this).html("-");
                    } else {
            ...
            $(".getOrderPembayaran").each(function () {
                ...
                if (!($(this).data("column-name") == orderBy["column_name"])) {
                    ...
                    if (orderBy["order"] == "asc" || orderBy["order"] == "no") {
