...
function rowEntry(control) {
    ...
    input_image =
        base_s3 +
        "/" +
        $(control).closest("tr").find(".pasien_image div").html();
    input_nama_asuransi = $(control)
        .closest("tr")
        .find(".nama_asuransi div")
        .html();

    input_ID = $(control).closest("tr").find("td:first-child div").html();
    input_asuransi_id = $(control)
        .closest("tr")
    ...
    input_prolanis_dm = $(control)
        ...
        .html();
    input_prolanis_ht = $(control)
        .closest("tr")
        .find(".prolanis_ht div")
        .html();
    return processEntry();
}
...
function processEntry() {
    ...
    if (input_asuransi_id != $("#asuransi_id_biaya_pribadi").val()) {
        input_option_asuransi +=
            '<option value="' +
            input_asuransi_id +
            '">' +
