...
function refreshTunaiPiutang() {
    ...
    } else if (!$("." + asuransi_coa_id)[0] && temp_coa_exist && piutang > 0) {
        temp[key_by_coa].nilai = piutang;
        render(temp);
    } else if (!$("." + asuransi_coa_id)[0] && temp_coa_exist && piutang < 1) {
    ...
    } else if (tunai > 0 && !$(".name_110")[0] && !coa_tunai_tersedia) {
        var newElement = {
            coa: "Kas di kasir",
            coa_id: "110000",
            nilai: tunai,
...
function delJurnal(control) {
    ...
    for (var i = 0, len = rows; i < len; i++) {
        $("#table_template_jurnal tbody tr:nth-child(" + parseInt(i + 1) + ")")
            .find(".key")
            .html(i);
    }
}
