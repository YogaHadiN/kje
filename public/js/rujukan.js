jQuery(document).ready(function ($) {
    ...
    $("#dummySubmit").click(function (e) {
        ...
        if (tujuan_rujuk == "" || rumah_sakit == "" || hamil_id == "") {
            // kenapa yang ini false?
    ...
    $("#G").keyup(function (e) {
        if ($(this).val() != "" && $(this).val() < 10) {
            $.post(
                ...
                { G: $(this).val(), pasien_id: $("#pasien_id").val() },
                function (data) {
                    $("#HPHT").val(data.hpht);
...
function tujuanChange(control) {
    ...
    if (tujuan_rujuk != "") {
        $.post(
            base + "/rujuajax/tujurujuk",
            { tujuan_rujuk: tujuan_rujuk, asuransi_id: asuransi_id },
            function (data, textStatus, xhr) {
                var temp = "";
                if (data.length > 0) {
                    ...
                    temp +=
                        "<td> Tidak Ada Data, atau pilih yang lain yang ada </td>";
                }
                var text =
                ...
                text +=
                    '<table class="table table-condensed table-bordered DTaja"><tbody>';
                text += "<thead><tr>";
                text += "<th>" + "Nama" + "</th>";
                text += "<th>" + "Rayon" + "</th>";
                text += "<th>" + "Jenis" + "</th>";
    ...
    if (rumah_sakit != "") {
        $.post(
            ...
            function (data, textStatus, xhr) {
                ...
                temp +=
                    '<textarea class="form-control" type="text" placeholder="Cari Info di Internet" name="rumah_sakit_alamat">' +
                    data.alamat +
                    "</textarea>";
                temp += "</div>";
                temp += '<div class="form-group">';
                ...
                temp +=
                    ...
                    data.telepon +
                    '" placeholder="Cari Info di Internet" name="rumah_sakit_telepon" />';
                temp += "</div>";
                temp += '<div class="form-group">';
                ...
                temp +=
                    '<input  type="text" class="form-control" value="' +
                    ugd +
                    '" placeholder="Cari Info di Internet" name="rumah_sakit_ugd" />';
                temp += "</div>";
                temp +=
                    '<br /><div class="alert alert-success"><h4>Isian ini secara otomatis akan tertulis di surat rujukan</h4></div>';
                var text =
                    '<div class="panel panel-default"><div class="panel-body">';
