    ...
    for (var i = 0; i < MyArray.length; i++) {
		 if( pertama_kali ){ 
		 	for (var r = 0; r < excel_pembayaran.length; r++) { 
                ...
                var nama_pasien_dicek = MyArray[i].nama_pasien.toLowerCase().replaceAll(' ', '').replaceAll('.', '');
                var nama_pasien       = excel_pembayaran[r][0].toLowerCase().replaceAll(' ', '').replaceAll('.', '');
                var piutang           = MyArray[i].piutang;
                var sudah_dibayar     = MyArray[i].pembayaran;

		 		if ( piutang - sudah_dibayar == excel_tagihan && 
                    (
...
function refreshExcelPembayaran(excel_pembayaran){
	...
	for (var r = 0; r < excel_pembayaran.length; r++) {
		...
		temp_excel_pembayaran += '<td class="nama_peserta">' + excel_pembayaran[r][0] + '</td>';
		temp_excel_pembayaran += '<td class="tagihan">' + excel_pembayaran[r][1] + '</td>';
