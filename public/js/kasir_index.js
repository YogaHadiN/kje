...
function viewTarif(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        temp +=
            '<td><input class="form-control money" dir="rtl" autocomplete="off" value="' +
            data[i].biaya +
            '" id="txt' +
            data[i].jenis_tarif.replace(/ /g, "") +
            i +
            '" onkeyup="inputTarif(this);" /></td>';
        if (data[i].jenis_tarif == "Biaya Obat") {
            temp +=
                '<td class="text-center"><button type="button" class="btn btn-danger btn-xs" disabled="disabled" value="' +
                i +
                '" onclick="rowDel(this)">hapus</button></td>';
            temp += "<tr>";
        } else {
            temp +=
                '<td class="text-center"><button type="button" class="btn btn-danger btn-xs" value="' +
                i +
                '" onclick="rowDel(this)">hapus</button></td>';
            temp += "<tr>";
        }
    ...
    if (dibayar_asuransi == "") {
        ...
        $("#dibayar_pasien").val(
            parseInt(totalBiaya) - parseInt(dibayar_asuransi)
        );
    }
...
function asuransiKeyup(event, control) {
    if (event.keyCode == 13) {
        ...
        if (dibayar_pasien_baru > 0) {
            ...
            $("#pembayaran_pasien").val(0).attr("readonly", "readonly");
...
function pembayaranKeyup(e, control) {
    ...
    if (e.keyCode == 13) {
        ...
        $("#kembalian_pasien").val(
            parseInt(pembayaran) - parseInt(dibayar_pasien)
        );
        rupiahDibayarPasien(control);
        rupiahDibayarPasien("#kembalian_pasien");
    }
...
function updateTotal(data) {
    ...
    if (dibayar_asuransi == "") {
        ...
        $("#dibayar_pasien").val(
            parseInt(total) - parseInt(clean(dibayar_asuransi))
        );
    }
