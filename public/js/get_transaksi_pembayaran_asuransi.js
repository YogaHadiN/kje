...
function getAsuransi(key = 0){
	...
	for (let i = 0, len = data.length; i < len; i++) {
		...
		temp += '/' ;
		temp += Date.parse(data[i].bulan).moveToLastDayOfMonth().toString("yyyy-MM-dd") ;
		temp += '" target="_blank"> ' ;
		temp += Date.parse(data[i].bulan).toString("yyyy-MM") ;
		temp += '</a>';
		temp += '</td>';
		if (
			parseInt(Date.parse(data[i].bulan).toString("dd")) == 1
		) {
			temp += '<td nowrap>';
			temp += '<a href="' ;
			temp += base ;
		...
		temp += '<td class="uang" nowrap>' + (parseInt(piutang) - parseInt(sudah_dibayar)) + '</td>';
		temp += '</tr>';
	}
	  $('#riwayatHutang').html(temp);
	  $('#namaAsuransi').html(
		  '<a href="' +base + '/asuransis/' + asuransi_id + '/hutang/pembayaran">Riwayat Hutang'+ nama_asuransi +'</a>'
	  );
	  $('#table_riwayat_hutang').find('.uang').each(function(){
...
function displaySpinner() {
		...
		var colspan = $('#riwayatHutang').closest('table').find('thead tr').find('th:not(.displayNone)').length;
		$('#riwayatHutang').html(
...
function asuransiChangeBulanan(key = 0){
  ...
  function getAsuransiBulanan(key = 0){
	displaySpinnerBulanan();
	  ...
	  $.post(base + '/pendapatans/pembayaran/asuransis/riwayatHutang/bulanan', param, function(data) {
		  var temp = '';
		  var total_rows = data.total_rows;
		  var data       = data.result;
		for (let i = 0, len = data.length; i < len; i++) {
			...
			temp += '<td class="uang" nowrap>' + sudah_dibayar + '</td>';
			temp += '<td class="uang" nowrap>' + (parseInt(data[i].piutang) - parseInt(sudah_dibayar)) + '</td>';
			temp += '</tr>';
		}
		  $('#riwayatHutangBulanan').html(temp);
		  $('#table_riwayat_hutang_bulanan').find('.uang').each(function(){
			  var money = uang( $(this).html() )
			  $(this).html(money);
		  });
		  var visible_pages = 7;
...
function displaySpinnerBulanan() {
	var colspan = $('#riwayatHutangBulanan').closest('table').find('thead tr').find('th:not(.displayNone)').length;
