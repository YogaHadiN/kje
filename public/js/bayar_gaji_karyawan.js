...
function dummySubmit() {
    ...
    if (validatePass() && tanggal_benar && kolomGajiTerisiSemua()) {
        $("#submit").click();
...
function tambah(control) {
    ...
    var jumlah_bonus = $(control)
        .closest("tr")
        .find(".jumlah_bonus_text")
        .val();
    var validated = true;

    if (nama_staf == "" || gaji_pokok == "" || jumlah_bonus == "") {
        validated = false;
        if (nama_staf == "") {
            validasi1(
                $(control).closest("tr").find(".nama_staf_select"),
                "Harus Diisi!"
            );
        }
        if (gaji_pokok == "") {
            validasi1(
                $(control).closest("tr").find(".gaji_pokok_text"),
                "Harus Diisi!"
            );
        }
        if (jumlah_bonus == "") {
            validasi1(
                $(control).closest("tr").find(".jumlah_bonus_text"),
                "Harus Diisi!"
            );
        }
    }
...
function changeNamaStaf(control) {
    ...
    if (typeof container_gaji[key] === "undefined") {
        container_gaji[key] = {
            staf_id: staf_id,
        };
    } else {
...
function kurang(control) {
    ...
    if (table_rows < 2) {
        ...
        for (var i = 0, len = table_rows; i < len; i++) {
            ...
            $(
                "#tabel_daftar_gaji tbody tr:nth-child(" +
                    parseInt(row_number) +
                    ")"
            )
                .find(".key")
