...
function clearAndSearch(key = 0){
	$("#pembayaran_asuransi_container").html("<tr><td colspan='" +length + "' class='text-center'><img class='loader' src='" + base_s3 + "/img/loader.gif'></td></tr>");
...
$(function () {
  ...
  function dummySubmit(){
	...
	function cariPembayaranAsuransi(key = 0){
		...
		var awal_periode       = $('#table_pembayaran_asuransi').find('.awal_periode').val();
		var akhir_periode      = $('#table_pembayaran_asuransi').find('.akhir_periode').val();
			...
			function (data, textStatus, jqXHR) {
				...
				if( data.data.length > 0 ){
					for (var i = 0; i < data.data.length; i++) {
						...
						temp += ''
						temp += '<td>'
						temp += '<a type="button" class="btn btn-primary btn-sm" href="' + base + '/pembayaran_asuransis/' + data.data[i].id + '" >Detail</button>';
						temp += '</td>'
						temp += '</tr>'
					}
				} else {
...
function datafunction(item, index) {
	for (let i = 0, len = item.length; i < len; i++) {
		...
		tempriwayathutang += '<td class="uang">' + item[i].sudah_dibayar + '</td>';
		tempriwayathutang += '<td class="uang">' + (parseint(item[i].piutang) - parseint(item[i].sudah_dibayar)) + '</td>';
		tempriwayathutang += '</tr>';
	}
}
