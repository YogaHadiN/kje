...
$("#dummyButton").click(function (e) {
    if ($("#antrianpoli_poli").val() == "usg") {
        var r = confirm(
            "Pastikan pasien diperiksa dalam keadaan kandung kencing penuh, UNTUK PASIEN USG DENGAN UMUMR KEHAMILAN 12 MINGGU"
        );
        if (!r) {
            return false;
        }
    ...
    if ($("#antrianpoli_poli").val() == "usgabdomen") {
        var r = confirm(
            "Pastikan pasien diperiksa dalam keadaan kandung kencing penuh"
        );
        if (!r) {
            return false;
        }
...
$("#submitPasien").click(function (e) {
    ...
    if (
        $('#pasienInsertForm input[name="nama"]').val() == "" ||
        $('#pasienInsertForm select[name="sex"]').val() == "" ||
        $('#pasienInsertForm select[name="panggilan"]').val() == ""
    ) {
        if ($('#pasienInsertForm input[name="nama"]').val() == "") {
            validasi(
                '#pasienInsertForm input[name="nama"]',
                "<code>Harus Diisi</code>"
            );
        }
        if ($('#pasienInsertForm select[name="sex"]').val() == "") {
            validasi(
                '#pasienInsertForm select[name="sex"]',
                "<code>Harus Diisi</code>"
            );
        }
        if ($('#pasienInsertForm select[name="panggilan"]').val() == "") {
            validasi(
                '#pasienInsertForm select[name="panggilan"]',
                "<code>Harus Diisi</code>"
            );
        }
        ...
        $.post(url, data, function (result) {
            ...
            if (DDID_PASIEN) {
                ...
                temp +=
                    "<td nowrap><div class='invisible'>" +
                    result[0].id +
                    "</div></td>";
            }
            temp +=
                "<td class='nama_pasien'><div class='invisible'>" +
                caseNama(result[0].nama) +
                "</div></td>";
            temp +=
                "<td nowrap><div class='invisible'>" +
                caseNama(result[0].alamat) +
                "</div></td>";
            temp +=
                "<td nowrap><div class='invisible'>" +
                ...
                "</div></td>";
            temp +=
                "<td nowrap class='no_telp'><div class='invisible'>" +
                result[0].no_telp +
                "</div></td>";
            if (DDID_ASURANSI) {
                temp +=
                    "<td nowrap class='displayNone'><div>" +
                    caseNama(result[0].nama_asuransi) +
                    "</div></td>";
            } else {
                temp +=
                    "<td nowrap><div class='invisible'>" +
            ...
            if (DDnamaPeserta) {
                temp +=
                    "<td nowrap class='displayNone'><div>" +
                    caseNama(result[0].nama_peserta) +
                    "</div></td>";
            } else {
                temp +=
                    "<td nowrap><div class='invisible'>" +
            ...
            temp +=
                ...
                "</div></td>";
            temp +=
                '<td nowrap nowrap><div class=\'invisible\'><a href="#" style="color: green; font-size: large;" onclick="rowEntry(this);return false;" data-toggle="tooltip" data-placement="bottom" title="Klik untuk periksa pasien"><span class="glyphicon glyphicon-log-in" aria-hidden="true"></span></a>';
            ...
            if (lewat_antrian > 0) {
                temp +=
                    ...
                    "/edit/" +
                    antrian_id +
                    '" style="color: ##337AB7; font-size: large;" data-toggle="tooltip" data-placement="bottom" title="Klik untuk ubah data pasien"><span aria-hidden="true" class="glyphicon glyphicon-edit"></span></a>';
            } else {
                temp +=
                    '&nbsp;&nbsp;&nbsp;<a href="pasiens/' +
                    result[0].id +
                    '/edit" style="color: ##337AB7; font-size: large;" data-toggle="tooltip" data-placement="bottom" title="Klik untuk ubah data pasien"><span aria-hidden="true" class="glyphicon glyphicon-edit"></span></a>';
            }
            temp +=
                "&nbsp;&nbsp;&nbsp;<a onclick='confirmStafModal();' data-value=\"pasiens/" +
                result[0].id +
                '" style="color: orange; font-size: large;" data-toggle="tooltip" data-placement="bottom" title="Klik untuk melihat riwayat pasien"><span class="glyphicon glyphicon-info-sign" aria-hidden="true"></span></a>';
...
$("body").on("keyup", ".ajaxselectpasien", function () {
    $("#processing-warning")
        .html(
            '<i class="fa fa-refresh fa-spin fa-fw"></i> <span class="sr-only">Loading...</span> Processing'
        )
        .removeAttr("class")
...
function selectPasien(key = 0) {
    ...
    $.get(url, data, function (hasil) {
        ...
        for (var i = 0; i < MyArray.length; i++) {
            ...
            if (DDID_ASURANSI) {
                temp +=
                    "<td nowrap class='displayNone nama_asuransi'><div>" +
                    caseNama(MyArray[i].namaAsuransi) +
                    "</div></td>";
            } else {
                temp +=
                    "<td nowrap class='nama_asuransi'><div>" +
                    caseNama(MyArray[i].namaAsuransi) +
                    "</div></td>";
            }
            if (DDnomorAsuransi) {
            ...
            temp +=
                "<td nowrap class='displayNone prolanis_dm'><div>" +
                MyArray[i].prolanis_dm +
                "</div></td>";
            temp +=
                "<td nowrap class='displayNone prolanis_ht'><div>" +
                MyArray[i].prolanis_ht +
                "</div></td>";
            temp +=
            ...
            if (lewat_antrian) {
                temp +=
                    ...
                    "/edit/" +
                    antrian_id +
                    '" style="color: ##337AB7; font-size: large;"><span aria-hidden="true" class="glyphicon glyphicon-edit"></span></a>';
            } else {
                temp +=
                    '&nbsp;&nbsp;&nbsp;<a href="pasiens/' +
                    MyArray[i].ID_PASIEN +
            ...
            temp +=
                "&nbsp;&nbsp;&nbsp;<a data-value='" +
                MyArray[i].ID_PASIEN +
                '\' onclick=\'confirmStafModal(this);\' href=\'#\' style="color: orange; font-size: large;" ><span class="glyphicon glyphicon-info-sign" aria-hidden="true"></span></a> ';
            temp +=
                '&nbsp;&nbsp;&nbsp;<a href="pasiens/' +
                MyArray[i].ID_PASIEN +
                '/transaksi" style="color: ##337AB7; font-size: large;"><i class=\'fas fa-money-bill-wave-alt\'></i></a>';
        ...
        .fail(function () {
            $(".ajax-error-pasien").removeClass("hide").hide().fadeIn(500);
            $("#processing-warning")
                .html(
        ...
        .done(function () {
            ...
            $("#processing-warning")
                .html('<i class="fa fa-check" aria-hidden="true"></i> Sukses')
                .removeAttr("class")
    ...
    $.post(base + "/pasiens/ajax/ajaxpasien", param, function (data) {
        ...
        if (data.antrian == "" && data.pasien == "") {
            ...
            if (data.antrian != "") {
                validasi(
                    "#antrianpoli_antrian",
                    "sudah ada antrian <br /> nama : " + data.antrian
                );
            }
            if (data.pasien != "") {
...
function cekPromo(control) {
    ...
    if (no_ktp.length == 16) {
            ...
            function (data, textStatus, jqXHR) {
                ...
                if (parseInt(data) > 0) {
                    ...
                    $(control)
                        .closest(".form-group")
                        .append(
                            '<span class="help-block">No KTP sudah digunakan Tahun ini</span>'
                        );
                } else {
                    ...
                    $(control)
                        .closest(".form-group")
                        .append(
                            '<span class="help-block">Promo bisa digunakan untuk No KTP ini</span>'
                        );
                }
            }
        );
        ...
        $(control)
            .closest(".form-group")
            .removeAttr("class")
            .addClass("form-group has-warning");
        $(control)
            .closest(".form-group")
            .append('<span class="help-block">KTP harus 16 digit</span>');
    }
}
function loaderGif() {
    var colspan = $("#ajax")
